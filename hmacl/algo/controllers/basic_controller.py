...
class BasicMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        if self.agent_output_type == "pi_logits":
            if getattr(self.args, "mask_before_softmax", True):
                # Give unavailable actions negligible softmax probability.
    ...
    def soft_update_model(self, path, tau):
        ...
        source_agent.load_state_dict(
            th.load(
                "{}/agent.th".format(path), map_location=lambda storage, loc: storage
            )
        )
        for target_param, param in zip(
            self.agent.parameters(), source_agent.parameters()
        ):
    ...
    def _get_input_shape(self, scheme):
        input_shape = int(np.prod(scheme["obs"]["vshape"]))
        if self.args.obs_last_action:
